...
# Set Together API Key from environment
...
2. Guide users through the claim filing process.
...
def handle_chat():
    ...
    if user_input.lower() in ['hi', 'hello', 'hey', 'restart', 'new conversation']:
        state.reset()
        return jsonify({"response": "Hello! How can I assist you with your insurance needs today?"})
    ...
    if "confirm" in user_input.lower():
        ...
        if 'policy_id' not in state.collected_data:
            ...
            if existing_claim:
                ...
                state.reset()  # Reset after showing existing claim
                ...
                if next_field in invalid_fields:
                    ...
                    if state.missing_fields:
                        ...
                        if db_handler.create_claim(state.collected_data):
                            ...
                            # Reset the state after successful claim creation
                            state.reset()
                            return jsonify({"response": response_message})
                        else:
